...
class IntentAnalysis(BaseModel):
    ...
    tool_input: Annotated[
        ...
        Field(
            default=None,
            description="The extracted input for the tool. For calculate: the mathematical expression (e.g., '1+3-4'). For web_search: a specific search query WITH date/location context (e.g., 'Houston events January 2026' not just 'events'). For define_word: the word to define. For get_upcoming_events: combine timeframe ('today', 'tomorrow', 'week', 'month'), keywords ('haircut', 'meeting'), temporal ('past', 'future'), or specific dates ('January 30') - e.g., 'haircut' to find a haircut appointment, 'past meeting' for past meetings. For add_to_calendar: the event name plus any date/time words, with pronouns resolved to the real event name (e.g. 'Punk Rock Garage Sale', 'dentist Tuesday 3pm'); for several events or a tentative status, a JSON object {'events': [{'title', 'when', 'location'}], 'status': 'tentative'} with one entry per event from the most recent assistant suggestions. For search_internal_docs: key terms about the feature (e.g., 'groups', 'calendar sync', 'events', 'knowledge graph', 'settings', 'getting started', 'tools capabilities')."
        )
    ]
